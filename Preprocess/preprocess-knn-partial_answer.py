...
pd.DataFrame(dataset).to_csv('train_knn.csv', index=False)
...
del testdata['Album_type'], testdata['Licensed'], testdata['official_video'],testdata['Key']
...
ansdata = deepcopy(testdata)

pd.DataFrame(testdata).to_csv('test_knn.csv', index=False)

# Stage2 : deal with the answers
ans = pd.read_csv('test_partial_answer.csv')
ansdata['id'] = tmpid
out = {}

out['Danceability'] = []
for i in ansdata.keys() :
    out[i] = []


for i in range(len(ansdata['id'])) :
    if ansdata['id'][i] in list(ans['id']):
        for j in list(ansdata.keys()) :
            out[j].append(ansdata[j][i])
        out['Danceability'].append( ans['Danceability'][ list(ans['id']).index(ansdata['id'][i]) ] )

del out['id']
pd.DataFrame(out).to_csv('ans_data.csv', index=False)
