...
for element in tqdm(itertools.product(*param)):
    ...
    for i in range(9) :
        ...
        svmModel.fit(train_x, train_y_bin)
        vy_bin.append(svmModel.predict(val_x))
...
for i in range(9) :
    ...
    svmModel_test.fit(np.concatenate([x , x_ans]), np.concatenate([y_bin , ans_y_bin]))
    yr.append(svmModel_test.predict(tx))
...
pd.DataFrame(output).to_csv('svm.csv', index=False)